...
get_performance_stats <- function(logf) {
    sample <-
        strsplit(logf, '/')[[1]] %>%
        last() %>%
        strsplit(., '.log') %>%
        ...
        tail(., 4) %>%
        paste(., collapse='_')
    replicate <- strsplit(logf, '/')[[1]] %>% tail(., 2) %>% head(., 1)
    logr <- read.delim(logf, header = FALSE)
    logr <- logr[,ncol(logr)]
    elapsed_time <-
        logr[grep('Elapsed', logr)] %>%
        strsplit(., ' ') %>%
        lapply(., as.ITime) %>%
        unlist() %>%
        na.exclude() %>%
    ...
    ram <-
        ...
        max() %>%
        suppressWarnings()
    return(c(replicate, sample, elapsed_time, ram))
}

collate_performance_stats <- function(folder, method) {
    pstats <- list.files(folder, pattern = '.log', full.names = TRUE, recursive = TRUE) %>%
        grep('clonal', ., value = TRUE, invert = TRUE) %>%
        lapply(., get_performance_stats) %>%
        data.frame() %>% t() %>%
        data.frame(., row.names = 1:nrow(.))
    colnames(pstats) <- c('replicate', 'sample', 'seconds', 'mem')
    # pstats$seconds <- hms(pstats$time) %>% period_to_seconds(.)
