load_ccube_results <- function(snv_folder, replicate, wd_truth, i) {
    mix <- paste(i, 1-i, sep='-')
...
get_dpc_clus_ccf <- function(id, clustering) {
    cloc <- clustering$cluster.locations[,1] == id
    clus_ccf <- clustering$cluster.locations[cloc, 2]
    if (any(is.na(clus_ccf))) {
        return(NA)
    }
    return(clus_ccf)
}
